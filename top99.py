...
def FE(nelx, nely, x, penal):
    ...
    for elx in range(1, nelx+1):
        for ely in range(1, nely+1):
            ...
            for j in edof - 1:
                for i in edof - 1:
                    K[i][j] = K[i][j] + x[ely-1, elx-1] ** penal * KE[i % 8][j % 8]

    F[(nely + 1) * (nelx + 1)-1, 0] = -1
    ...
    for i in range(U.shape[1]):
        U[freedofs-1, i] = np.linalg.inv(K[freedofs-1][:, freedofs-1]).dot(F[freedofs-1, i])
    U[fixeddofs-1][:, :] = np.zeros(U[fixeddofs-1][:, :].shape)
    return U
